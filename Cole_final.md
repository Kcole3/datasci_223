...
* This model was then adapted to enable randomized weights which are then trained through gradient descent
...
* The ReLu activation function was used which returns the value of itself when the input is positive, and zero otherwise. It helps to introduce non-linearity into the model, which is why it was selected.
...
* We note an improvement in both the loss and the C-statistic with an increase in the depth of the hidden layer with the best architecture being
...
* The first hidden layer was arbitrarily selected to be 2X the input layer, with the 2nd hidden layer selected to be equal to the input layer (ie 15, 30, 15, 1)
* We now see that increasing both the depth AND the number of hidden layers leads to an improvement in the C-statistic, now 0.807


### Step 8- Latent Class Analysis

* A clustering approach to identify hidden subgroups was pursued via LCA. Each subgroup was then individually trained in the architecture of the  best GNN identified above. 
* LCA was performed on 9 latent classes. The number of classes were determined based on the number of distinct cancer subtypes represented in the dataset (Small cell lung cancer, non-small cell lung cancer, melanoma, hepatobiliary cancer, gastric cancer, pancreatic cancer, renal cancer, bladder, esophgeal, and head and neck cancer). The C statistic was **********
* Following this the 10 latent classes were trained on the best GNN (15,30,15,1). 
